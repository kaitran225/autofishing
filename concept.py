...
class PixelChangeDetector(QObject):
    ...
    def __init__(self):
        ...
        self.enable_repair_detection = True    # Toggle for repair dialog detection
        self.enable_action_sequence = True     # Toggle for action sequence execution
        self.pending_reference_capture = False # Flag for delayed reference capture
        self.reference_capture_time = 0        # Timestamp for when to capture reference
        ...
        self.action_sequence = [
            ...
            {"action": "schedule_reference_capture", "delay": 1.5}  # Schedule a reference capture after 1.5s
        ]
    ...
    def calculate_adaptive_threshold(self, frame):
        ...
        if mean > 160:  # Bright scene
            return max(10, std * 0.5)  # Lower threshold for bright scenes
        elif mean < 60:  # Dark scene
            return max(25, std * 1.0)  # Higher threshold for dark scenes
        else:
            return max(15, std * 0.7)  # Medium threshold for normal scenes
            
    def analyze_scene(self, frame):
        """Analyze scene characteristics to adjust detection parameters"""
        if frame is None:
            return
        ...
        if mean > 160:  # Bright scene
            ...
            self.blur_kernel_size = 3
            # Log if there was a change in mode
            if not hasattr(self, '_last_scene_type') or self._last_scene_type != 'bright':
                self.log(f"Scene analysis: Bright scene detected (mean:{mean:.1f}, std:{std:.1f})")
                self._last_scene_type = 'bright'
        elif mean < 60:  # Dark scene
            self.enhanced_bright_detection = False
            self.blur_kernel_size = 5
            # Log if there was a change in mode
            if not hasattr(self, '_last_scene_type') or self._last_scene_type != 'dark':
                self.log(f"Scene analysis: Dark scene detected (mean:{mean:.1f}, std:{std:.1f})")
                self._last_scene_type = 'dark'
        else:  # Normal scene
            self.enhanced_bright_detection = True
            self.blur_kernel_size = 3
            # Log if there was a change in mode
            if not hasattr(self, '_last_scene_type') or self._last_scene_type != 'normal':
                self.log(f"Scene analysis: Normal scene detected (mean:{mean:.1f}, std:{std:.1f})")
                self._last_scene_type = 'normal'
                
        # Adjust threshold based on scene variation - more dynamic but within bounds
        scene_threshold = max(0.03, min(0.15, std / 1000.0))
        
        # Only update if significant change to avoid oscillation
        if abs(scene_threshold - self.THRESHOLD) > 0.01:
            self.log(f"Adjusting threshold from {self.THRESHOLD:.3f} to {scene_threshold:.3f} based on scene")
            self.THRESHOLD = scene_threshold
    ...
    def calculate_frame_difference(self, frame1, frame2):
        ...
        # Analyze scene and adjust parameters if needed
        self.analyze_scene(frame1)
            
        # Use a fixed base threshold but with adaptivity from scene analysis
        # This combines the simplicity of the sample code with our adaptive capabilities
        threshold_base = 30  # Base value from sample code (higher base for clearer detection)
        if hasattr(self, '_last_scene_type'):
            if self._last_scene_type == 'bright':
                threshold_base = 25  # Slightly lower for bright scenes
            elif self._last_scene_type == 'dark':
                threshold_base = 35  # Higher for dark scenes
        
        if self.enhanced_bright_detection:
            # Apply CLAHE (Contrast Limited Adaptive Histogram Equalization) to improve contrast
            # Using simpler approach from sample code
            ...
            # Identify bright areas with simpler fixed threshold
            # Using 180 from the sample code
            ...
            if np.any(bright_mask):
                # Apply a more sensitive threshold to bright areas - simpler approach
            ...
            # Standard detection method with improved sensitivity
            diff_frame = cv2.absdiff(frame1, frame2)
            _, thresholded_diff = cv2.threshold(diff_frame, threshold_base, 255, cv2.THRESH_BINARY)
        
        # Add temporal smoothing if we have a previous diff frame
        if hasattr(self, 'last_diff_frame') and self.last_diff_frame is not None:
            # Ensure shapes match first
            if self.last_diff_frame.shape == thresholded_diff.shape:
                # Blend with previous frame to reduce flickering (70% current, 30% previous)
                thresholded_diff = cv2.addWeighted(
                    thresholded_diff, 0.7, 
                    self.last_diff_frame, 0.3, 
                    0
                )
        
        # Store for next comparison
        self.last_diff_frame = thresholded_diff.copy()
        
        # Use smaller kernel for noise reduction as in sample code
    ...
    def capture_reference(self):
        """Capture a reference frame with validation for stability"""
        if not self.region:
            self.log("You must select a region first")
            return False
            
        try:
            # Capture multiple frames to find the most stable one
            frames = []
            for i in range(3):
                frame = self.capture_screen()
                if frame is not None:
                    frames.append(self.color_frame.copy())  # Store color frame
                    if i < 2:  # Don't sleep after last capture
                        time.sleep(0.1)
            
            if not frames:
                self.log("Failed to capture any valid frames for reference")
                return False
                
            if len(frames) > 1:
                # Calculate variance between consecutive frames as stability metric
                variances = []
                for i in range(len(frames) - 1):
                    if len(frames[i].shape) == 3:
                        ...
                        variance = np.sum(diff ** 2) / diff.size
                        variances.append(variance)
                
                if variances:
                    # Select the frame that leads to minimum variance (most stable)
                    best_idx = np.argmin(variances)
                    self.reference_frame = frames[best_idx]
                    # Log the stability information
                    self.log(f"Reference frame captured (stability: {variances[best_idx]:.2f})")
                else:
                    # If we couldn't calculate variances, use the last frame
                    self.reference_frame = frames[-1]
                    self.log("Reference frame captured (no stability calculation)")
            else:
                # Only got one frame
                self.reference_frame = frames[0]
                self.log("Reference frame captured (single frame)")
    ...
    def _detection_loop(self):
        """Main detection loop with improved recovery mechanisms"""
        # Initialize local variables for performance
        local_threshold = self.THRESHOLD
        local_interval = self.capture_interval
        consecutive_no_change = 0
        max_consecutive_no_change = 10  # Reset reference frame after this many unchanged frames
        
        # Add adaptive timing variables
        min_interval = 0.005  # Minimum 5ms between captures
        max_interval = 0.05   # Maximum 50ms between captures
        target_cpu_usage = 0.4  # Target 40% CPU usage per thread
        process_times = []  # Track processing times for averaging
        
        # Initialize for temporal analysis
        change_history_buffer = []
        ...
        while self.is_running and not self.stop_requested:
            try:
                ...
                # Check if we need to capture a reference frame (delayed capture)
                current_time = time.time()
                if self.pending_reference_capture and current_time >= self.reference_capture_time:
                    self.log("Executing delayed reference frame capture")
                    self.capture_reference()
                    self.pending_reference_capture = False  # Reset flag
                ...
                # Store in local buffer for temporal analysis
                change_history_buffer.append(change_percent)
                if len(change_history_buffer) > 10:
                    change_history_buffer = change_history_buffer[-10:]
                
                # Emit signal for UI update at reduced frequency
                if not hasattr(self, '_last_ui_update') or (current_time - getattr(self, '_last_ui_update', 0)) > 0.033:
                    self.frame_updated.emit()
                    self._last_ui_update = current_time
                
                # Simplified detection condition with cooldown period
                # Similar to sample code but maintaining our cooldown logic
                if (change_percent > local_threshold and 
                    ...
                    consecutive_no_change = 0  # Reset counter on detection
                    
                    # Only start action sequence if enabled
                    if self.enable_action_sequence:
                        ...
                        self.log("Action sequence disabled, skipping execution")
                        self.detection_signal.emit()
                        
                        # Schedule delayed reference frame capture even if action sequence is disabled
                        self.pending_reference_capture = True
                        self.reference_capture_time = current_time + 1.5  # 1.5 seconds
                        self.log("Scheduled reference frame capture in 1.5s")
                    
                    continue
                else:
                    # Increment counter for no change
                    consecutive_no_change += 1
                    
                    # Adaptive reference frame reset based on time and change level
                    max_no_change_time = 30  # seconds
                    if ((consecutive_no_change >= max_consecutive_no_change) or
                            (current_time - last_significant_change_time > max_no_change_time)):
                        self.log(f"No significant changes detected for a while ({current_time - last_significant_change_time:.1f}s), resetting reference frame...")
                        self.capture_reference()
                        consecutive_no_change = 0
                        last_significant_change_time = current_time
                ...
                if len(process_times) > 10:
                    process_times = process_times[-10:]
                
                # Adaptive timing based on processing load
                avg_process_time = np.mean(process_times)
                load_factor = avg_process_time / local_interval
                
                # Adjust interval based on load
                if load_factor > target_cpu_usage:
                    # Increase interval (slow down) if too much CPU is used
                    local_interval = min(local_interval * 1.05, max_interval)
                elif load_factor < target_cpu_usage * 0.7:
                    # Decrease interval (speed up) if CPU usage is low
                    local_interval = max(local_interval * 0.95, min_interval)
                
                # Adaptively adjust detection cooldown based on system performance
                self.detection_cooldown = max(0.3, min(1.0, avg_process_time * 10))
                
                # Control capture rate with adaptive interval
                sleep_time = max(0, local_interval - process_time)
                if sleep_time > 0:
                ...
                if self.consecutive_failures > self.max_consecutive_failures:
                    ...
                    self.previous_frame = None
                    self.diff_frame = None
                    self.reference_frame = None  # Reset reference frame
        ...
        try:
            ...
            if self.action_sequence_step == 0 and action_type == "press_f":
                ...
                if action_type == "press_f":
                    ...
                    # Skip repair dialog check if disabled
                    if not self.enable_repair_detection:
                        self.log(f"Action sequence: Skipping repair dialog check (disabled)")
                    else:
                        ...
                        if not self._check_repair_dialog():
                            ...
                            self.log("No repair dialog found, continuing sequence")
                            # Skip ahead to the next action after the repair button click
                            self.action_sequence_step += 2  # Skip repair button click
                elif action_type == "click_repair_button":
                    # Skip if repair detection is disabled
                    if not self.enable_repair_detection:
                        self.log(f"Action sequence: Skipping repair button click (disabled)")
                    else:
                        self.log(f"Action sequence: Clicking repair button")
                        self._click_repair_button()
                elif action_type == "schedule_reference_capture":
                    # Schedule the reference frame capture for 1-2 seconds later
                    self.log(f"Action sequence: Scheduling reference frame capture in {action['delay']}s")
                    self.pending_reference_capture = True
                    self.reference_capture_time = time.time() + action['delay']
            ...
            if self.action_sequence_step >= len(self.action_sequence):
                ...
                # We no longer take a new reference frame immediately
                # Instead we've scheduled it for later with schedule_reference_capture
        except Exception as e:
            ...
            if self.action_sequence_step >= len(self.action_sequence):
                ...
                # Cancel any pending reference capture
                self.pending_reference_capture = False
    ...
    def capture_screen_background(self):
        ...
        try:
            ...
            try:
                ...
                if "permission" in error_message or "not allowed" in error_message or "denied" in error_message:
                    # Display an error directly on the overlay
                    error_pixmap = QPixmap(self.screen_width, self.screen_height)
                    error_pixmap.fill(QColor(44, 36, 23))  # Dark oak background
                    
                    painter = QPainter(error_pixmap)
                    painter.setPen(QColor(248, 244, 227))  # Cream text color
                    painter.setFont(QFont("Helvetica", 16, QFont.Weight.Bold))
                    
                    # Draw error message
                    painter.drawText(
                        QRect(50, 50, self.screen_width - 100, self.screen_height - 100),
                        Qt.AlignmentFlag.AlignCenter | Qt.TextFlag.TextWordWrap,
                        "SCREEN RECORDING PERMISSION ERROR\n\n"
                        "This application needs screen recording permission to function properly.\n\n"
                        "Please check System Settings > Privacy & Security > Screen Recording\n"
                        "and ensure this application is allowed."
                    )
                    painter.end()
                    
                    # Show this error background instead
                    return error_pixmap
                    
                # For other errors, create a plain background
                fallback_pixmap = QPixmap(self.screen_width, self.screen_height)
                fallback_pixmap.fill(QColor(44, 36, 23))  # Dark oak background
                return fallback_pixmap
                
        except Exception as e:
            print(f"Error capturing screen background: {e}")
            # Create empty pixmap as fallback
            empty_pixmap = QPixmap(self.screen_width, self.screen_height)
            empty_pixmap.fill(QColor(44, 36, 23))  # Dark oak background
            return empty_pixmap
...
class PixelChangeApp(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # Add a second row of checkboxes for the new toggleable features
        checkbox_layout2 = QHBoxLayout()
        checkbox_layout2.setSpacing(24)  # Reduced spacing between checkboxes
        checkbox_layout2.setContentsMargins(0, 0, 0, 0)  # No margins
        
        # Repair detection checkbox
        self.repair_checkbox = QCheckBox("Repair Detection")
        self.repair_checkbox.setStyleSheet("font-size: 12px;")
        ...
        self.action_checkbox.setStyleSheet("font-size: 12px;")
        ...
        """)
        permissions_button.setMinimumHeight(26)  # Shorter height
    ...
    def toggle_repair_detection(self, checked):
        """Toggle repair detection processing"""
        if self.detector:
    ...
    def toggle_action_sequence(self, checked):
        """Toggle action sequence execution"""
        if self.detector:
