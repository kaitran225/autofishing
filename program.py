...
from PyQt6.QtGui import (QFont, QColor, QPalette, QPixmap, QFontMetrics, QCursor, QMouseEvent, QImage,
                        QPainter, QPen, QMoveEvent, QScreen)
...
class PixelChangeDetector:
    def __init__(self, log_queue=None):
        ...
        self.gui: Optional["OverlayAutoFisher"] = None
        
        # Screen capture region (required)
        self.region: Optional[Tuple[int, int, int, int]] = None  # (left, top, right, bottom)
    ...
    def focus_play_together_window(self):
        ...
        try:
            if self.play_together_window is None or not win32gui.IsWindow(self.play_together_window):
    ...
    def is_fullscreen_app_active(self):
        ...
        try:
            ...
            if hasattr(self, 'gui') and self.gui:
                if hasattr(self.gui, 'winId'):
                    if foreground_hwnd == int(self.gui.winId()):
                        return False
            ...
            try:
                ...
                if time.time() - fps_timer >= 1.0:
                    ...
                    if time.time() - last_fps_update >= 5.0:
                        if self.gui:
                            QTimer.singleShot(0, lambda f=fps: self.gui.log(f"Processing at {f} FPS"))  # type: ignore
                        last_fps_update = time.time()
                ...
                if change_percent > self.THRESHOLD and (current_time - self.last_detection_time) > self.detection_cooldown:
                    ...
                    if self.gui:
                        QTimer.singleShot(0, self.gui.increment_detection_count)  # type: ignore
    ...
    def _handle_detection(self):
        ...
        try:
            ...
            while time.time() < pause_end and self.is_running and not self.thread_control.get("stop_requested", False):
                ...
                if self.gui:
                    if hasattr(self.gui, 'status_label') and self.gui.status_label is not None:
                        QTimer.singleShot(0, lambda r=remaining: self.gui.status_label.setText(f"System: monitor.paused ({r}s)"))  # type: ignore
                time.sleep(0.1)
...
class RegionSelectorOverlay(QMainWindow):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            left = pos.x() - self.region_width // 2
            top = pos.y() - self.region_height // 2
    ...
    def paintEvent(self, event):
        ...
        painter.drawLine(0, screen_height // 2, screen_width, screen_height // 2)
        
        # Vertical guide line
        painter.drawLine(screen_width // 2, 0, screen_width // 2, screen_height)
        
        # Draw instruction panel at top center
        instruction_text = "SELECT REGION • CLICK TO PLACE • ESC TO CANCEL"
        ...
        painter.fillRect(panel_rect, QColor(0, 0, 0, 180))
        ...
        if self.current_point is not None:
            ...
            painter.fillRect(QRect(left, top, self.region_width, self.region_height), Qt.GlobalColor.transparent)
            
            # Reset composition mode for drawing
            ...
            # Vertical grid lines
            cell_width = self.region_width // 3
            for i in range(1, 3):
                painter.drawLine(
            ...
            cell_height = self.region_height // 3
            for i in range(1, 3):
                painter.drawLine(
...
class OverlayAutoFisher(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.detector = PixelChangeDetector(log_queue=self.log_queue)
        self.detector.gui = self  # type: ignore  # Set the GUI reference for thread-safe callbacks
    ...
    def on_threshold_changed(self, value):
        ...
        if self.threshold_var < 0.05:
            self.add_log("Current threshold is very sensitive - may cause false positives")
          
        elif self.threshold_var > 0.25:
            self.add_log(f"Current threshold: {self.threshold_var} is not very sensitive - may miss subtle changes")
    ...
    def apply_settings(self):
        """Apply settings to the monitoring system"""
        # Get values from UI
        try:
            threshold = self.threshold_var
            cooldown = float(self.cooldown_entry.text())
            fishing_key = self.fishing_key_entry.text()
            region_size = int(self.size_entry.text())
            
            # Validate values
            if threshold < 0.01 or threshold > 0.5:
                self.add_log("Threshold must be between 0.01 and 0.5")
                return
            
            if cooldown < 0.1 or cooldown > 30.0:
                self.add_log("Cooldown must be between 0.1 and 30 seconds")
                return
            
            if not fishing_key or len(fishing_key) > 1:
                self.add_log("Fishing key must be a single character")
                return
            
            # Store values
            self.threshold_var = threshold
            self.cooldown_var = str(cooldown)
            self.fishing_key_var = fishing_key
            
            # If we have a region and the size has changed, update the region
            if self.region and region_size > 10:
                left, top, right, bottom = self.region
                current_width = right - left
                
                if current_width != region_size:
                    # Calculate new region maintaining the center point
                    center_x = (left + right) // 2
                    center_y = (top + bottom) // 2
                    
                    # Calculate aspect ratio
                    aspect_ratio = (bottom - top) / current_width
                    
                    # Calculate new height based on aspect ratio
                    new_height = int(region_size * aspect_ratio)
                    
                    # Calculate new region
                    new_left = center_x - (region_size // 2)
                    new_top = center_y - (new_height // 2)
                    new_right = new_left + region_size
                    new_bottom = new_top + new_height
                    
                    # Update region
                    self.region = (new_left, new_top, new_right, new_bottom)
                    self.add_log(f"Region resized to {region_size}x{new_height}")
    ...
    def stop(self):
        ...
        if self.preview_timer:
            self.preview_timer.setInterval(self.preview_interval)
    ...
    def pause(self):
        ...
        if hasattr(self.detector, 'thread_control'):
            self.detector.thread_control['paused'] = self._paused
    ...
    def select_region(self):
        """Open a region selection overlay to select a screen region"""
        self.add_log("Starting region selection...")
        
        try:
            # Get the size from the input field
            size = int(self.size_entry.text())
            if size < 10:
                self.add_log("Size must be at least 10 pixels")
                return
        except ValueError:
            self.add_log("Invalid size value. Using default of 50 pixels")
            size = 50
    ...
    def start_preview(self):
        ...
        if not self.preview_active:
            ...
            self.add_log("Preview started")
    
    def stop_preview(self):
        """Stop updating the preview image"""
        if self.preview_timer is not None:
            self.preview_timer.stop()
            self.preview_active = False
            self.add_log("Preview stopped")
    ...
    def update_preview(self):
        ...
        # Create QImage with a copy of the data to avoid memoryview issues
        q_img = QImage(rgb_frame.data.tobytes(), w, h, bytes_per_line, QImage.Format.Format_RGB888)
        
        # Scale to fit preview area while maintaining aspect ratio
        preview_width = self.preview_label.width() - 10  # Margin
        preview_height = self.preview_label.height() - 10  # Margin
    ...
    def restore_after_region_selection(self, was_minimized):
        """Restore window state after region selection"""
