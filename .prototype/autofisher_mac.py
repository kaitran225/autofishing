...
from PyQt6.QtWidgets import (
    ...
    QGroupBox, QMessageBox, QDialog, QDialogButtonBox, QCheckBox, QComboBox, QGridLayout
)
...
class PixelChangeDetector(QObject):
    ...
    def __init__(self):
        ...
        self.game_app_name = None  # Store the game application name
        self.play_together_window = None  # Window information
        ...
        # Updated action sequence with timing specified by user
        self.action_sequence = [
            {"action": "press_f", "delay": 0.0},    # Press fishing key and wait 5 seconds (cooldown)
            {"action": "wait", "delay": 5.0},       # Wait for remainder of cooldown (already waited full cooldown)
            {"action": "press_esc", "delay": 0.0},
            {"action": "wait", "delay": 2.0},       # Press ESC key and wait 1 second
    ...
    def perform_health_check(self):
        ...
        # Check if we haven't had a successful capture in a while (extended to 5 minutes)
        if self.last_successful_capture > 0 and (current_time - self.last_successful_capture) > 300.0:
    ...
    def _detection_loop(self):
        ...
        while self.is_running and not self.stop_requested:
            try:
                ...
                current_time = time.time()
                if (current_time - self._last_health_check) > 300.0:
    ...
    def _process_action_sequence(self):
        ...
        if action["action"] == "press_f":
            self.log(f"Action sequence: Pressing fishing key")
            self._send_f_key()
        ...
        if self.action_sequence_step >= len(self.action_sequence):
            ...
            self.log("Capturing new reference frame after fishing sequence")
            self.capture_reference()
            
    def _send_f_key(self):
        """Send F key press using AppleScript - with proper window focusing"""
        try:
            # Check if we have a game application name
            if self.game_app_name:
                # First try to focus the specific game
                script = f'''
                try
                    tell application "{self.game_app_name}" to activate
                    delay 0.1
                    tell application "System Events"
                        tell process "{self.game_app_name}"
                            keystroke "f"
                        end tell
                    end tell
                    return true
                ...
                if result.stdout.strip().startswith("Error:"):
                    self.log(f"Error with targeted keystroke: {result.stdout.strip()}")
                    # Fallback to generic keystroke
                    generic_script = '''
                    tell application "System Events"
                        keystroke "f"
                    end tell
                    '''
                    subprocess.run(['osascript', '-e', generic_script], check=True, capture_output=True)
                    self.log("Used fallback keystroke method")
            else:
                # No game name identified, use generic keystroke
                self.log("Game application not identified, using generic keystroke")
                script = '''
                tell application "System Events"
                    keystroke "f"
                end tell
    ...
    def _send_esc_key(self):
        """Send ESC key press using AppleScript - with proper window focusing"""
        try:
            # Check if we have a game application name
            if self.game_app_name:
                # First try to focus the specific game
                ...
                # Check if it worked or if there was an error
                if result.stdout.strip().startswith("Error:"):
                    self.log(f"Error with targeted ESC keystroke: {result.stdout.strip()}")
                    # Fallback to generic keystroke
                    generic_script = '''
                    tell application "System Events"
                        key code 53  # ESC key code
                    ...
                    '''
                    subprocess.run(['osascript', '-e', generic_script], check=True, capture_output=True)
                    self.log("Used fallback ESC keystroke method")
            else:
                # No game name identified, use generic keystroke
                self.log("Game application not identified, using generic ESC keystroke")
                script = '''
                tell application "System Events"
                    key code 53  # ESC key code
                end tell
...
class RegionSelectionOverlay(QDialog):
    ...
    def __init__(self, parent=None, default_size=50):
    ...
    def find_play_together_window(self):
        ...
        try:
            # AppleScript to find windows with PLAY TOGETHER in the title and get the process name
            script = '''
            tell application "System Events"
                repeat with proc in processes
                    repeat with w in windows of proc
                        if name of w contains "PLAY TOGETHER" or name of w contains "Play Together" or name of w contains "play together" then
                            ...
                            return {item 1 of winPos, item 2 of winPos, item 1 of winSize, item 2 of winSize, procName}
                        end if
                    end repeat
                end repeat
                return ""
            ...
            if result.stdout.strip():
                try:
                    # Parse the result into left, top, width, height, app_name
                    ...
                    if len(values) >= 5:
                        ...
                        width = int(values[2])
                        height = int(values[3])
                        app_name = values[4].strip()
                        
                        self.play_together_rect = QRect(left, top, width, height)
                        self.game_app_name = app_name
                        print(f"Found game window: {app_name} at {left},{top} ({width}x{height})")
                        return True
                except Exception as e:
            ...
            tell application "System Events"
                ...
                repeat with proc in processes
                    repeat with w in windows of proc
                        set windowInfo to windowInfo & {name of w, name of proc}
                    end repeat
                end repeat
                return windowInfo
            end tell
            ...
            print(f"Window info: {result2.stdout}")
...
class TimelinePlot(FigureCanvas):
    ...
    def __init__(self, parent=None, width=4, height=1, dpi=80):
        # Create figure and axes with smaller dimensions
        self.fig, self.ax = plt.subplots(figsize=(width, height), dpi=dpi)
        self.fig.set_facecolor('#181914')  # Match app bg_dark color
        self.ax.set_facecolor('#181914')
        
        # Set tight layout to minimize margins
        ...
        # Create the plot with thinner lines
        self.activity_line, = self.ax.plot(self.x_data, self.y_data, color='#A3D977', linewidth=0.8)
        self.threshold_line = self.ax.axhline(y=0.05, color='#FF4D4D', linestyle='--', alpha=0.6, linewidth=0.8)
        
        # Configure appearance
        self.ax.set_xlim(0, 99)
        self.ax.set_ylim(0, 1)
        self.ax.set_xticks([])
        self.ax.grid(False)  # Remove grid for cleaner look
        
        # Small title with no padding
        self.ax.text(0.5, 0.85, "activity", color='#A3D977', fontsize=7, 
                  ha='center', va='bottom', transform=self.ax.transAxes)
        
        # Set text color for axis labels and ticks - smaller font
        self.ax.tick_params(axis='y', colors='#A3A08C', labelsize=6)
        ...
        self.setMinimumHeight(60)  # Set minimum height to avoid excessive shrinking
        
    def update_theme(self, colors):
        """Update colors to match the app theme"""
        self.fig.set_facecolor(colors['bg_dark'])
        self.ax.set_facecolor(colors['bg_dark'])
        self.activity_line.set_color(colors['accent'])
        self.threshold_line.set_color(colors['alert'])
        self.ax.tick_params(axis='y', colors=colors['text_dim'])
        self.ax.texts[0].set_color(colors['accent'])
        self.draw()
...
class PixelChangeApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("AutoFisher - Mac Edition v1.0")
    ...
    def _define_colors(self):
        ...
        # Define enhanced color scheme inspired by matcha and oak wood theme
        self.colors = {
            ...
            'text_bright': '#FFFFFF',     # Bright white for emphasis
            ...
            'green_alt': '#BCD9B4',       # Lighter green
            'success': '#47D068',         # Success indicator
            'alert': '#FF4D4D',           # Alert/error
            'warning': '#FFB940',         # Warning indicator
            'border': '#6B6E58',          # Border color
            'border_light': '#A3A08C',    # Lighter border
            'cursor': '#A3D977',          # Cursor color
        }
    ...
    def _init_ui(self):
        ...
        self.setStyleSheet(f"""
            ...
            QTextEdit {{ 
                ...
                font-size: 9px;
                padding: 2px;
                selection-background-color: {self.colors['accent_alt']};
        ...
        left_panel = QWidget()
        left_panel.setFixedWidth(180)  # Reduce width further
        ...
        # More compact layout for sliders
        settings_grid = QGridLayout()
        settings_grid.setVerticalSpacing(1)
        settings_grid.setHorizontalSpacing(2)
        
        # Threshold control
        threshold_label = QLabel("threshold:")
        threshold_label.setFixedWidth(55)
        ...
        self.threshold_value_label.setStyleSheet("font-size: 8px;")
        ...
        self.size_value_label.setStyleSheet("font-size: 8px;")
        self.size_value_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        settings_grid.addWidget(self.size_value_label, 1, 2)
        
        # Add cooldown control (new feature)
        cooldown_label = QLabel("cooldown:")
        cooldown_label.setFixedWidth(55)
        settings_grid.addWidget(cooldown_label, 2, 0)
        
        self.cooldown_slider = QSlider(Qt.Orientation.Horizontal)
        self.cooldown_slider.setRange(1, 30)  # 1 to 30 seconds
        self.cooldown_slider.setValue(5)     # Default 5 seconds
        self.cooldown_slider.valueChanged.connect(self.update_cooldown)
        settings_grid.addWidget(self.cooldown_slider, 2, 1)
        
        self.cooldown_value_label = QLabel("5.0s")
        self.cooldown_value_label.setFixedWidth(30)
        self.cooldown_value_label.setStyleSheet("font-size: 8px;")
        self.cooldown_value_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        settings_grid.addWidget(self.cooldown_value_label, 2, 2)
        
        settings_layout.addLayout(settings_grid)
        
        # Add fishing key configuration (new feature)
        fishing_label = QLabel("fishing_key:")
        fishing_label.setFixedWidth(55)
        settings_grid.addWidget(fishing_label, 3, 0)
        
        self.key_combo = QComboBox()
        self.key_combo.addItems(["F", "E", "Space", "R"])
        self.key_combo.setCurrentIndex(0)  # Default is F
        settings_grid.addWidget(self.key_combo, 3, 1)
        ...
        stats_grid.setHorizontalSpacing(2)
        stats_grid.setColumnStretch(1, 1)  # Make second column expandable
        
        # Create stat labels
        self.stats_labels = {
            "total_detections": QLabel("0"),
            "runtime": QLabel("00:00:00"),
            "rate": QLabel("0.0/hr"),
            "avg_interval": QLabel("0s"),
            "fps": QLabel("0")
        }
        
        # Add stats to grid - with smaller fonts
        font_style = "font-size: 8px;"
        
        stats_grid.addWidget(QLabel("Detections:"), 0, 0)
        self.stats_labels["total_detections"].setStyleSheet(font_style)
        stats_grid.addWidget(self.stats_labels["total_detections"], 0, 1, Qt.AlignmentFlag.AlignRight)
        
        stats_grid.addWidget(QLabel("Runtime:"), 1, 0)
        self.stats_labels["runtime"].setStyleSheet(font_style)
        stats_grid.addWidget(self.stats_labels["runtime"], 1, 1, Qt.AlignmentFlag.AlignRight)
        
        stats_grid.addWidget(QLabel("Rate:"), 2, 0)
        self.stats_labels["rate"].setStyleSheet(font_style)
        stats_grid.addWidget(self.stats_labels["rate"], 2, 1, Qt.AlignmentFlag.AlignRight)
        
        stats_grid.addWidget(QLabel("Interval:"), 3, 0)
        self.stats_labels["avg_interval"].setStyleSheet(font_style)
        stats_grid.addWidget(self.stats_labels["avg_interval"], 3, 1, Qt.AlignmentFlag.AlignRight)
        
        stats_grid.addWidget(QLabel("FPS:"), 4, 0)
        self.stats_labels["fps"].setStyleSheet(font_style)
        stats_grid.addWidget(self.stats_labels["fps"], 4, 1, Qt.AlignmentFlag.AlignRight)
        
        stats_layout.addLayout(stats_grid)
        
        # Add a timer to update statistics
        self.stats_timer = QTimer()
        self.stats_timer.timeout.connect(self.update_statistics)
        self.stats_timer.start(1000)  # Update every second
        
        # 3. Control group with integrated monitoring
        control_group = QGroupBox("CONTROL")
        control_layout = QVBoxLayout(control_group)
        control_layout.setContentsMargins(4, 8, 4, 4)
        control_layout.setSpacing(1)
        left_layout.addWidget(control_group)
        
        # Region selection buttons
        region_layout = QHBoxLayout()
        region_layout.setSpacing(1)
        ...
        self.reference_button = QPushButton("capture-ref")
        self.reference_button.clicked.connect(self.capture_reference)
        region_layout.addWidget(self.reference_button)
        
        control_layout.addLayout(region_layout)
        
        # Region info display
        region_info_layout = QHBoxLayout()
        region_info_layout.setSpacing(1)
        region_info_layout.addWidget(QLabel("status:"))
        self.region_info_label = QLabel("waiting...")
        self.region_info_label.setStyleSheet("font-size: 8px;")
        region_info_layout.addWidget(self.region_info_label)
        control_layout.addLayout(region_info_layout)
        ...
        right_layout.addWidget(viz_group, stretch=3)
        
        self.monitor_display = MonitoringDisplay()
        viz_layout.addWidget(self.monitor_display, stretch=3)
        
        # Add info bar below monitor display
        info_layout = QHBoxLayout()
        info_layout.setSpacing(2)
        
        # Add coordinate info
        self.coords_label = QLabel("coordinates: (waiting for selection)")
        self.coords_label.setStyleSheet(f"color: {self.colors['accent_special']}; font-family: Menlo, monospace; font-size: 8px;")
        info_layout.addWidget(self.coords_label)
        
        # Add change percentage display
        self.change_percent_label = QLabel("change: 0.00%")
        self.change_percent_label.setStyleSheet(f"color: {self.colors['accent']}; font-family: Menlo, monospace; font-size: 8px;")
        self.change_percent_label.setAlignment(Qt.AlignmentFlag.AlignRight)
        info_layout.addWidget(self.change_percent_label)
        ...
        timeline_layout.setContentsMargins(4, 8, 4, 4)
        timeline_layout.setSpacing(2)
        right_layout.addWidget(timeline_group, stretch=1)
        
        self.timeline_plot = TimelinePlot(width=4, height=1)
        timeline_layout.addWidget(self.timeline_plot)
        
        # Add threshold info below timeline
        threshold_info = QHBoxLayout()
        threshold_info.setSpacing(2)
        
        threshold_info_label = QLabel("Detection threshold:")
        threshold_info_label.setStyleSheet("font-size: 8px;")
        threshold_info.addWidget(threshold_info_label)
        
        self.timeline_threshold_label = QLabel(f"{self.detector.THRESHOLD:.2f}")
        self.timeline_threshold_label.setStyleSheet(f"color: {self.colors['alert']}; font-weight: bold; font-size: 8px;")
        threshold_info.addWidget(self.timeline_threshold_label)
        
        # Add spacer to push items left and right
        threshold_info.addStretch()
        
        # Add timestamp of last detection
        self.last_detection_label = QLabel("Last: --:--:--")
        self.last_detection_label.setStyleSheet("font-size: 8px;")
        threshold_info.addWidget(self.last_detection_label)
        
        timeline_layout.addLayout(threshold_info)
        
        # 3. Status and count display
        status_panel = QFrame()
        status_panel.setStyleSheet(f"background-color: {self.colors['bg_dark']}; border: 1px solid {self.colors['border']};")
        status_panel.setMaximumHeight(20)  # Make status bar smaller
        status_layout = QHBoxLayout(status_panel)
        status_layout.setContentsMargins(4, 2, 4, 2)
        status_layout.setSpacing(2)
        
        self.status_label = QLabel("system:monitor.idle")
        self.status_label.setStyleSheet(f"color: {self.colors['text_dim']}; font-family: Menlo, monospace; font-size: 8px;")
        status_layout.addWidget(self.status_label)
        
        self.count_label = QLabel("detections: 0")
        self.count_label.setStyleSheet(f"color: {self.colors['accent']}; font-family: Menlo, monospace; font-size: 8px;")
        status_layout.addWidget(self.count_label, alignment=Qt.AlignmentFlag.AlignRight)
        
        right_layout.addWidget(status_panel)
        
        # Add log display under timeline
        ...
        self.log_display.setMaximumHeight(80)  # Limit height to keep UI compact
        self.log_display.setStyleSheet("font-size: 7px; font-family: Menlo, Consolas, monospace;")
        log_layout.addWidget(self.log_display)
        
        # Add version info at the bottom
        version_info = QLabel("AutoFisher v1.0 - Mac Edition © 2025")
        version_info.setStyleSheet(f"color: {self.colors['text_dim']}; font-size: 7px; padding: 0px;")
        version_info.setAlignment(Qt.AlignmentFlag.AlignRight)
        version_info.setMaximumHeight(10)
        right_layout.addWidget(version_info)
        
        # Add a welcome log message
        self.add_log("AutoFisher - Mac Edition v1.0 initialized")
    ...
    def update_cooldown(self):
        """Update cooldown value from slider"""
        value = self.cooldown_slider.value()
        self.cooldown_value_label.setText(f"{value}.0s")
        
        if self.detector:
            self.detector.detection_cooldown = value
            self.log(f"Detection cooldown updated to {value} seconds")
    
    def dummy_function(self):
        """Placeholder for features to be implemented later"""
        self.log("This feature will be implemented in a future update")
    ...
    def update_statistics(self):
        ...
        # Update stats
        self.stats_labels["total_detections"].setText(str(self.detection_count))
        self.stats_labels["runtime"].setText(runtime_str)
        
        # Calculate detection rate (per hour)
        if runtime > 0:
            rate = (self.detection_count / runtime) * 3600
            self.stats_labels["rate"].setText(f"{rate:.1f}/hr")
        
        # Calculate average interval
        if self.detection_count > 1:
            avg_interval = runtime / max(1, self.detection_count)
            ...
            if interval_mins > 0:
                ...
                interval_str = f"{interval_secs}s"
                
            self.stats_labels["avg_interval"].setText(interval_str)
            
        # Update FPS if detector is running
        if self.detector and hasattr(self.detector, 'performance'):
            if hasattr(self.detector.performance, 'fps'):
                fps = self.detector.performance.get('fps', 0)
                self.stats_labels["fps"].setText(str(int(fps)))
            else:
                # Fallback calculation if fps isn't directly available
                avg_time = self.detector.performance.get('avg_processing_time', 0.1)
                if avg_time > 0:
                    fps = int(1.0 / avg_time)
                    self.stats_labels["fps"].setText(str(fps))
    ...
    def select_region(self):
        ...
        self.selection_overlay = RegionSelectionOverlay(None, region_size)
        self.selection_overlay.region_selected.connect(self.set_region)
        
        # Force the selection dialog to stay on top
        self.selection_overlay.setWindowFlag(Qt.WindowType.WindowStaysOnTopHint, True)
        ...
        result = self.selection_overlay.exec()
    ...
    def set_region(self, region):
        ...
        # If we've selected a window from RegionSelectionOverlay, transfer the window info to detector
        if hasattr(self, 'selection_overlay') and hasattr(self.selection_overlay, 'game_app_name') and self.selection_overlay.game_app_name:
            self.detector.game_app_name = self.selection_overlay.game_app_name
            self.add_log(f"Game window detected: {self.detector.game_app_name}")
    ...
    def handle_detection(self):
        ...
        timestamp = datetime.datetime.now().strftime("%H:%M:%S")
        self.last_detection_label.setText(f"Last: {timestamp}")
    ...
    def update_visualization(self):
        ...
        # Update coordinate label if region is selected
        if self.detector.region:
            left, top, right, bottom = self.detector.region
            width = right - left
            height = bottom - top
            self.coords_label.setText(f"coordinates: ({left},{top}) {width}×{height}")
        
        # Update change percentage label
        self.change_percent_label.setText(f"change: {current_change:.2%}")
        
        # Update threshold label
        self.timeline_threshold_label.setText(f"{self.detector.THRESHOLD:.2f}")
...
def main():
    ...
    # Size the window to 50% of the screen size (reduced from 80%)
    window_width = min(int(screen_geometry.width() * 0.5), 700)
    window_height = min(int(screen_geometry.height() * 0.5), 500)
